...
public class Raytrace extends Applet
{
   ...
   Color bodenFarbe(double[] s, double[] v)            // Bodenfarbe in einer Blickrichtung v
   {   if ( v[2] >= 0 ) return backColor;              // Strahl trifft nicht auf Boden
       double[] n = { 0, 0, 1};                        // Normale der Bodenebene
       double[] x = pprojektion(s, v, n, zBoden);      // Spurpunkt des Strahls auf Boden
       double r =  Math.sqrt(x[0]*x[0] + x[1]*x[1]);   // Abstand vom Nullpunkt
       if ( r > 20 )
          return backColor;
       int m = (int)r;
       if ( m % 2 == 0 ) return Color.gray;            // Punkt in dunklem Ring
       else  return Color.lightGray;                   // Punkt inh hellem Ring
   }
   ...
   {  Dimension dim = getSize();                          // Window-Abmessungen
      ...
      double[] p = vadd(vsub(m, u), v);                   // Referenzpukt in Bildebene (left,top)
      double[] du = vmult(2.0/(cols-1), u);               // Gitterabstand in Richtung u
      double[] dv = vmult(2.0/(lines-1), v);              // Gitterabstand in Richtung v
      ...
      {   p1 = vadd(p, vmult(-i,dv));                     // Startpunkt Zeile i
          ...
          {  gitterPkt = vadd(p1, vmult(j,du));           // Gitterpunkt  p - i*dv + j*du
             v = vsub(gitterPkt,s);                       // Richtung des Strahls
             color = bodenFarbe(s, v);                    // Bodenfarbe im Auftreffpunkt des Strahls
